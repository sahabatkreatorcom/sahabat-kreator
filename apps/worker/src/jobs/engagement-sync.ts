...
import { socialAccount, engagementItem } from "@sahabatkreator/db/schema";
...
function classifySentiment(text: string): Sentiment {
  ...
  const positiveWords = [
    "bagus", "keren", "suka", "mantap", "hebat", "top", "luar biasa", "wah",
    "nice", "love", "great", "amazing", "awesome", "perfect", "fantastic",
    "thanks", "thank", "makasih", "terima kasih", "sempurna", "bagus banget",
  ];
  const negativeWords = [
    "buruk", "jelek", "benci", "kesal", "kecewa", "sial", "jahat",
    "bad", "hate", "awful", "terrible", "worst", "disappointed", "angry",
    "mati", "bisa", "tidak", "nggak", "enggak",
  ];
...
function classifyType(comment: PlatformComment | PlatformMention, hint?: string): "COMMENT" | "MENTION" | "DM" | "REVIEW" | "STORY_MENTION" | "STORY_REPLY" | "PUBLIC_POST" {
  ...
  if (/[\ud83d\udcf1\ud83d\udcac]/.test((comment as { text: string }).text)) return "DM";
  return "COMMENT";
}
 ...
 */
function threadKey(postId: string | undefined, authorId: string): string {
...
async function fetchComments(
  ...
  if (platform === "INSTAGRAM") {
    ...
    // No media_id available without a prior query; return empty and log
...
async function fetchMentions(
  ...
  if (platform === "INSTAGRAM") {
    ...
    return getInstagramMentions(accessToken, limit) as unknown as PlatformMention[];
  }
...
async function upsertEngagementItem(
  ...
  const existing = await db
    ...
    .from(engagementItem)
    .where(and(eq(engagementItem.organizationId, orgId), eq(engagementItem.platform, platform as any), eq(engagementItem.content, (item as { text: string }).text)))
    .limit(1);
  ...
  await db
    ...
    .values([{
      ...
      type,
      platformAccountId: typeof item === "object" && "authorId" in item ? (item as { authorId: string }).authorId : undefined,
...
export const engagementSyncWorker = new Worker<EngagementSyncJobData>(
  ...
  async (job: Job<EngagementSyncJobData>) => {
    ...
    for (const comment of comments) {
      ...
      await upsertEngagementItem(organizationId, platform, comment, "COMMENT", comment.platformPostId);
      itemsProcessed++;
    }
    ...
    console.log(`[EngagementSync] Processed ${itemsProcessed} new items for ${platform}/${socialAccountId}`);
    return { synced: true, itemsProcessed, platform, socialAccountId, timestamp: new Date().toISOString() };
  },
  {
    connection,
    concurrency: 5,
