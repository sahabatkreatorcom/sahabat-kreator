...
export function MetaAPITests() {
  ...
  const testPlatform = async (platform: string) => {
    ...
    try {
      ...
      setResults((prev) =>
        prev.map((r) =>
          r.platform === platform ? { platform, status: "error", message: "Network error" } : r,
        ),
      );
    }
  };
    ...
    <div className="space-y-6">
              ...
              {result?.details && result.status === "success" && (
                <div className="mb-4 space-y-2 rounded-lg bg-[var(--bg-tertiary)] p-3 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span
                      className={cn(
                        result.details.accessTokenValid
                          ? "text-[var(--success)]"
                          : "text-[var(--error)]",
                      )}
                    >
                  ...
                  {result.details.followers && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {result.details.followers.toLocaleString("id-ID")}
                      </span>
                    </div>
                  )}
                </div>
      ...
      <div className="rounded-lg border border-[var(--warning)]/20 bg-[var(--warning)]/5 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-medium text-[var(--warning)] text-sm">Informasi Penting</p>
            <p className="mt-1 text-[var(--text-muted)] text-xs">
              Test ini hanya memverifikasi koneksi API dengan token yang tersedia. Pastikan
              credential platform sudah dikonfigurasi di tab "Platform API" sebelum melakukan test.
            </p>
          </div>
        </div>
      </div>
