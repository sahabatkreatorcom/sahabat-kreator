...
const TONES = [
  ...
  { value: "humorous", label: "Lucu", emoji: "😄" },
  { value: "inspirational", label: "Inspiratif", emoji: "✨" },
] as const;

const GOALS = [
  { value: "engagement", label: "Engagement", emoji: "💬" },
  { value: "followers", label: "Followers", emoji: "👥" },
  { value: "sales", label: "Penjualan", emoji: "💰" },
...
export default function SuggestionsPage() {
  ...
  const [recentContext, setRecentContext] = useState<{
    recentPosts: { caption?: string; postType?: string; daysAgo: number }[];
  ...
  useEffect(() => {
    async function fetchContext() {
      const res = await suggestionApi.getContent();
      if (res.ok) {
        setRecentContext(res.data);
      }
    }
    fetchContext();
  }, []);
  ...
  async function generateSuggestions() {
    ...
    const res = await suggestionApi.generateContent({
      ...
      tone: tone as "casual" | "professional" | "humorous" | "inspirational",
      goal: goal as "engagement" | "followers" | "sales" | "awareness",
    });
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <div className="card space-y-4 p-6">
        <div>
          <label htmlFor="topic" className="mb-2 block font-medium text-sm">
            Topik / Ide Konten
          </label>
          <textarea
            id="topic"
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div>
            <label htmlFor="platform" className="mb-2 block font-medium text-sm">
              Platform (Opsional)
            </label>
            <select
              id="platform"
