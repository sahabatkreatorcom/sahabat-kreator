...
export async function getCircuitStatus(platform: string): Promise<CircuitStatus> {
  ...
  try {
    ...
    return {
      ...
      failureCount: failures ? Number.parseInt(failures as string, 10) : 0,
      lastFailureAt: null, // Not tracked separately — failures list has timestamps
      openedAt: openedAt ? Number.parseInt(openedAt as string, 10) : null,
      halfOpenSuccesses: halfOpenOk ? Number.parseInt(halfOpenOk as string, 10) : 0,
    };
  } catch (error) {
...
export async function isCallAllowed(platform: string): Promise<boolean> {
  ...
  try {
    ...
    if (opened) {
      const elapsed = Date.now() - Number.parseInt(opened as string, 10);
    ...
    logger.error("Failed to check circuit breaker — allowing call (fail-open)", {
      platform,
      error,
    });
    return true;
...
export class CircuitOpenError extends Error {
  ...
  constructor(platform: string) {
    super(
      `Circuit breaker is OPEN for ${platform} — platform appears degraded, skipping to protect system`,
    );
