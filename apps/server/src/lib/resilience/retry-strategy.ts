    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            if (classification.retryability === 'permanent') {
                log?.warn(`Permanent error — not retrying (${classification.category})`, { attempt, maxAttempts, category: classification.category, ...logContext });
                throw error;
            }

            // Last attempt — throw without waiting
            if (attempt >= maxAttempts) {
                log?.error(`All ${maxAttempts} attempts exhausted`, { attempt, maxAttempts, category: classification.category, ...logContext });
                throw error;
            }
            ...
            log?.warn(`Retryable error — attempt ${attempt}/${maxAttempts}, waiting ${Math.round(delayMs)}ms`, { attempt, maxAttempts, delayMs: Math.round(delayMs), category: classification.category, ...logContext });
