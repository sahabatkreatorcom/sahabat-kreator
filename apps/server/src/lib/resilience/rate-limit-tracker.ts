...
export class RateLimitError extends Error {
  ...
  constructor(status: RateLimitStatus) {
    super(
      `Rate limit exceeded for ${status.platform} — ${status.used}/${status.limit} requests used`,
    );
    this.name = "RateLimitError";
    this.status = status;
    this.retryAfter = status.resetAt
...
export async function getRateLimitStatus(
  ...
  try {
    const [used, resetAt] = await Promise.all([redis.get<number>(uKey), redis.get<number>(rKey)]);
